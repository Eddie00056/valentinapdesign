...
const fractionMessage = (v: "v1" | "v2" | "hint", qty: string) => {
  if (v === "v1") return `Please use market order to trade ${qty} shares`;
  if (v === "v2") return "To place a fractional trade use Market orders";
  return `Switch to market order to buy ${num(qty)} shares, as limit orders don’t support fractional trading yet.`;
};
...
/* prefix + the typed figure + a blinking caret. `selected` tints the figure as
...
export function FractionalOrderFlow({
  ...
  /* The first field trades places on an order-type change: the old one clears
     before the new one lands, so the two labels never overprint. */
  const swapOut = reduced ? { duration: 0 } : { duration: 0.12, ease: "easeOut" as const };
  const swapIn = reduced ? { duration: 0 } : { ...SPRING, delay: 0.1 };
  ...
  /* the edge case: a fraction typed into a Limit quantity is refused. Every
     variant waits for the whole value ("1.5", not the "1." on its way there) */
          ...
          {isEntry && (
            ...
            >
              ...
              >
                ...
                {/* fractional shares are a dollars-in idea; a limit is placed in shares */}
                ...
                <AnimatePresence initial={false} mode="popLayout">
                  {isLimit && (
                    <motion.div
                      ...
                      exit={{ opacity: 0, y: -8, transition: swapOut }}
                      transition={swapIn}
                      style={{
                        ...fieldBase,
