...
/* Order placement — second screen in the stock-detail flow, reached from the
   quote screen's "Trade" action. Same PhoneFrame chrome, same live price
   clock (pxHub / PX_BASE), same DASH ticker, same UP/DOWN palette, so the
   two screens read as one continuous prototype. */
...
export function OrderPlacementScreen() {
    ...
    <PhoneStage>
      <PhoneFrame fadeAt={100}>
        <div style={{ position: "relative" }}>
          ...
          <style>{`.op-frac-btn svg { width: 16px; height: 16px; }`}</style>
          ...
          {/* order form — one continuous list of borderless, divider-separated
              rows (no boxed text fields): label left, value + a spinner caret
              right. Matches a real order-ticket's row language instead of
              the earlier bordered-input treatment. */}
          ...
          {/* scrim while the quantity-type menu is open, matching the reference */}
          <div style={{ position: "relative" }}>
            <Row label="Order type" value="Market" caret />
            <Row label="Route" value="Auto" caret />
            <Row label="Bracket order" value="None" caret />
            <Row label="Special instructions" value="None" caret />
            <Row label="Account" value="Individual Margin" caret />
            <Row
          ...
          {/* footer — the site's shared glass-pill buttons (glasslab/GlassButton),
              same red/green semantics as everywhere else on the site. */}
          <div className="dark" style={{ marginTop: 22, display: "flex", gap: 10 }}>
...
/** One row of the order form: label left, value (+ optional caret/nav
    chevron) right, hairline divider below — no boxed text fields. Pass
    `right` for a fully custom right side (e.g. the quantity stepper). */
function Row({
  label,
  value,
  right,
  ...
  return (
    <div
      style={{
        ...
        gap: 12,
        height: 50,
        borderBottom: last ? "none" : `1px solid ${BORDER_DIM}`,
      }}
    >
...
function ChevronRightIcon() {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="none" aria-hidden="true">
      <path d="M9 5l7 7-7 7" stroke={VAL} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
